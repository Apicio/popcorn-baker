...
SET MOVEFROM=%nodejsWork%\popcorn-app-%PT_REPO%\dist\windows
SET VERBOSE=0
...
ECHO.
IF EXIST "%INSTALLERWIN%\installer.nsi" DEL /S /Q "%INSTALLERWIN%\installer.nsi"

:: PREPARE CSCRIPT NSIS-SCRIPT
ECHO WScript.StdOut.Write "Download " ^& "%nsisUrl%" ^& " ">%nsisVbs%
ECHO dim http: set http = createobject("WinHttp.WinHttpRequest.5.1") >>%nsisVbs%
ECHO dim bStrm: set bStrm = createobject("Adodb.Stream") >>%nsisVbs%
ECHO http.Open "GET", "%nsisUrl%", True >>%nsisVbs%
ECHO http.Send >>%nsisVbs%
ECHO while http.WaitForResponse(0) = 0 >>%nsisVbs%
ECHO   WScript.StdOut.Write "." >>%nsisVbs%
ECHO   WScript.Sleep 1000 >>%nsisVbs%
ECHO wend >>%nsisVbs%
ECHO WScript.StdOut.WriteLine " [HTTP " ^& http.Status ^& " " ^& http.StatusText ^& "]" >>%nsisVbs%
ECHO with bStrm >>%nsisVbs%
ECHO .type = 1 '//binary >>%nsisVbs%
ECHO .open >>%nsisVbs%
ECHO .write http.responseBody >>%nsisVbs%
ECHO .savetofile "%INSTALLERWIN%\installer.nsi", 2 >>%nsisVbs%
ECHO end with >>%nsisVbs%

:: DOWNLOAD NSIS-SCRIPT
cscript.exe /NoLogo %nsisVbs%
...
:: MOVE INSTALLER
IF EXIST "%MOVEFROM%\PopcornTime*.exe" MOVE /Y "%MOVEFROM%\PopcornTime*.exe" "%PUB%"
...
:RUNSETUP
START "" "%PUB%\PopcornTimeSetup.exe" >NUL 2>&1
